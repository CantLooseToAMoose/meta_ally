...
#   timestamp: 2025-11-03T08:54:31+00:00
...
class AiKnowledgePlugin(BaseModel):
    ...
    query_tool_description: Optional[str] = Field(
        'Search a text database for contents similar to a given query string. The most relevant text fragments will be returned separated by `---`.  Each result may include a link to the source document with its name and URL in Markdown format.',
    ...
    metadata_tool_description: Optional[str] = Field(
        'Retrieve document-level metadata from a text database. The results will list documents per collection.  If the collection is large, only a subset will be included.',
