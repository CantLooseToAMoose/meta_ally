...
def create_variants_for_dataset(
    dataset, num_variants: int = 2, show_visualizations: bool = True
):
    """
    ...
    Returns:
        dict: Dictionary mapping case names to their original and variant cases
    """
    console.print(
        f"\n[bold yellow]Creating {num_variants} variants for each of "
        f"{len(dataset.cases)} test cases...[/bold yellow]\n"
    )
    ...
    for case_idx, case in enumerate(dataset.cases, 1):
        ...
        all_variants[case.name] = {"original": original_case, "variants": variants}
...
def main():
    ...
    all_variants = create_variants_for_dataset(
        dataset=dataset, num_variants=num_variants, show_visualizations=show_visualizations
    )
    ...
    console.print(
        f"[bold green]✓ Successfully created {total_variants} variants "
        f"across {len(all_variants)} test cases[/bold green]"
    )
