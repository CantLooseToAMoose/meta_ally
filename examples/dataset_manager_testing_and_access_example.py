"""
Example usage of DatasetManager with Testing & Access Management cases.

This example demonstrates:
1. Creating a HookLibrary with API operation hooks for test resource management
2. Loading cases from case_factory_testing_and_access_example
3. Creating datasets with 3 variants for each case
4. Assigning appropriate pre-task hooks to manage API resources (evaluation suites, roles)
...
from examples.case_factory_testing_and_access_example import (
    example_testing_and_access_management,
)
...
def main():
    ...
    print("=" * 80)
    print("Testing & Access Management DatasetManager Example - Save and Load Manager State")
    print("=" * 80)

    # Step 1: Load cases from case_factory_testing_and_access_example
    print("\n[1] Loading cases from case_factory_testing_and_access_example...")
    original_dataset = example_testing_and_access_management()
    ...
    for i, case in enumerate(original_dataset.cases, 1):
        ...
        dataset_id = manager.create_dataset_from_case(
            ...
            dataset_id=f"testing_access_case_{i}",
            num_variants=3,
            name=case.name,
            description=f"Testing & Access Management test case {i} with variants"
        )

        # Set appropriate hooks based on the case
        # Case 3: Tests creating evaluation suite - need to empty suite first
        # Case 5: Tests creating role with user - need to remove user and role first
        if i == 3:  # Case 3: Create Suite and Add Cases
            manager.set_dataset_hooks(
                dataset_id=dataset_id,
                pre_task_hook_id="empty_evaluation_suite"
            )
            print("        ✓ Created with 3 variants + cleanup hook (empties evaluation suite)")
        elif i == 5:  # Case 5: Add Colleague Access
            manager.set_dataset_hooks(
                dataset_id=dataset_id,
                pre_task_hook_id="cleanup_role_and_user"
            )
            print("        ✓ Created with 3 variants + cleanup hook (removes user and role)")
        else:
    ...
    output_dir = Path(__file__).parent.parent / "Data" / "testing_and_access"
